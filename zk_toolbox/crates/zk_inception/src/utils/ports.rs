...
impl EcosystemPortsScanner {
    ...
    fn scan_directory(dir: &Path, ecosystem_ports: &mut EcosystemPorts, skip_dirs: &HashSet<&str>) -> Result<()> {
    ...
    fn traverse_yaml(value: &Value, path: &str, file_path: &Path, ecosystem_ports: &mut EcosystemPorts) {
        match value {
            Value::Mapping(map) => {
                for (key, val) in map {
                    ...
                    if key.as_str().map(|s| s.ends_with("port")).unwrap_or(false) {
                        if let Some(port) = val.as_u64().and_then(|p| u16::try_from(p).ok()) {
                            ecosystem_ports.add_port_info(port, PortInfo {
